...
flatten_alpha_grm <- function(alpha, H) {
  ...
  list(
    alpha_flat = as.double(alpha_flat),
...
sj_ab_grm <- function(j) {
  ...
  h <- .subset2(y, j)
  drv_h <- vapply(theta_ls, function(theta_k) {
    exp(alpha[[j]][h] + beta[[j]] *
      theta_k) / (1 + exp(alpha[[j]][h] + beta[[j]] * theta_k))^2
  }, double(N))
  drv_h_plus_one <- -vapply(theta_ls, function(theta_k) {
    exp(alpha[[j]][h +
  ...
  s_alpha <- vapply(
    1:N, function(i) comp_a[i, ] %*% temp2[i, , 2:H[[j]]],
    double(H[[j]] - 1L)
  ) # (H[j]-1)*N matrix
...
sj_ab_grmDIF <- function(j) {
  ...
  if (j %in% items_dif) {
    drv_h <- vapply(
      theta_ls, function(theta_k) {
        exp(alpha[[j]][h] + beta[[j]] * theta_k +
          x0DIF(theta_k) %*% eta[[j]]) /
    ...
    drv_h_plus_one <- -vapply(
      theta_ls, function(theta_k) {
        exp(alpha[[j]][h + 1L] + beta[[j]] * theta_k +
          x0DIF(theta_k) %*% eta[[j]]) /
